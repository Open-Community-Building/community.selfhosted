...
Give every source a uniform, queryable **index of its items** — for each
item a stable *locator*, an MD5 *fingerprint*, and a few *features* — so that
sampling/test-sets, cross-source duplicate detection, dataset statistics, and
parallel processing are written **once, source-agnostically**, on top of a single
artifact instead of being re-implemented per source.
...
- **Fingerprint**: the MD5 (hex) of an item's canonical content bytes, used for
...
- **Manifest**: a table with one row per item — `(seq, kind, locator, fingerprint,
  size, features…)` — persisted as SQLite, indexing a source independently of any
  later conversion.
...
2. The generic **manifest stage** consumes the iterator and, per item, computes the
   MD5 over the content bytes, records the byte size, and appends a manifest row
   `(seq, kind, locator, fingerprint, size, features…)`.
3. **Sub-items** (e.g. attachments) are emitted by the iterator as their own rows,
   carrying their parent's locator, so content is fingerprinted at the granularity
...
2. A manifest MAY therefore be produced two ways: as a **standalone stage** (when
   ...
   **by-product of conversion** in a single pass. This is what keeps the universal
   index/fingerprint step from forcing a separate pre-pass onto every command.
...
2. Indexing is **deterministic**: re-running over an unchanged source yields the
   same locators and fingerprints, so manifests are comparable over time and
   support incremental / resume.
...
2. It is a **prerequisite artifact**: statistics, sampling, dedup and conversion all
   *read* it. It stores addresses + fingerprints + features only — never a copy of
   the source's content.

### Duplicate detection

1. Fingerprints are computed over **comparable units**, so the same bytes appearing
   in two sources (an email attachment and the same file in a photo set) share a
   fingerprint.
2. Duplicate detection is "group manifest rows by `fingerprint`" — within one
   manifest, or UNION-ed across many.
...
- A **manifest** table for the source — rows of `(seq, kind, locator, fingerprint,
  size, features…)` — in SQLite.

## Constraints

- Indexing is streaming, memory-bounded, read-only on the source, and deterministic.
- `fingerprint` = MD5 (hex) of the item's canonical content bytes; `size` (bytes) is
  stored alongside.
- The **item iterator is the only per-source code**; the manifest, fingerprint and
...
- Define the **canonical content** for fingerprinting non-file items (e.g. an email
...
- Where does the generic implementation live — the manifest/fingerprint/stats
  stages plus the **Source / item-iterator** contract — in **memex** (the
  source→SQLite package) or in **community.selfhosted** (which already hosts the
  photo/iOS pipelines this would absorb)? This spec is deliberately neutral on it.
